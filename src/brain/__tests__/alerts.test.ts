...
describe('brain/alerts', () => {
  ...
  describe('brain_ingest_stale', () => {
    it('fires when emails_seen_by_brain_24h=0 AND last_ingest > 6h ago', async () => {
      const { setSystemState: sss } = await import('../metrics.js');
      // Last event well over 6h ago; counter left untouched so it reads 0.
      const old = '2026-04-23T03:00:00Z';
      sss('last_ingest_event_at', old, old);
      const fired = evaluateAlerts({ nowIso: '2026-04-23T10:00:00Z' });
      expect(fired.map((a) => a.category)).toContain('brain_ingest_stale');
    });

    it('does NOT fire on a fresh install (no last_ingest_event_at)', () => {
    ...
    it('does NOT fire when emails were seen in the last 24h', async () => {
      ...
      sss('last_ingest_event_at', lastEvent, lastEvent);
