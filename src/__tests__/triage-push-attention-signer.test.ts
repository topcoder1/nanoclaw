...
describe('pushAttentionItem — signer ceremony', () => {
  ...
  it('flag OFF (no signerCeremonyId) → URL button (legacy behavior)', async () => {
    ...
    >;
    // Top row: Sign (URL) + Full Email (URL). Full Email is always added
    // by push-attention when MINI_APP_URL is set.
  ...
  it('flag ON + signerCeremonyId provided → callback_data buttons', async () => {
    ...
    const keyboard = opts.reply_markup.inline_keyboard as Array<
      Array<{ text: string; url?: string; callback_data?: string }>
    >;
    // Top row: Sign callback, Cancel Sign callback, Full Email URL button.
    expect(keyboard.length).toBe(2);
    expect(keyboard[0]).toHaveLength(3);
    const [approveBtn, cancelBtn, fullEmailBtn] = keyboard[0];
    ...
    expect(fullEmailBtn.text).toContain('Full Email');
    expect(fullEmailBtn.url).toContain('/email/');
